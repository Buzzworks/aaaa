...
function initial_dialler_state(){
    if ($("#feedback_tab_link").hasClass("disabled")) {
        ...
        $("#MDPhonENumbeR").val("")
        $("#profile-tab, #btnNextCall, #agent-callbacks-div button, #profile-tab,#show-callbacks-active,#show-callbacks-campaign,#show-abandonedcalls-campaign, #show-campaign-lead-bucket, #show-campaign-assigned-calls, #show-camp-requeue-lead-bucket, #show-camp-assigned-dialed-calls, #show-camp-assigned-notdialed-calls").removeClass("disabled")
        ...
        if (session_details[extension]['Unique-ID'] || call_type == 'webrtc') {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#MDPhonENumbeR").val("")
                    $("#profile-tab, #btnNextCall, #show-callbacks-active, #show-callbacks-campaign, #show-abandonedcalls-campaign, #show-campaign-lead-bucket, #show-campaign-assigned-calls, #show-camp-requeue-lead-bucket, #show-camp-assigned-dialed-calls, #show-camp-assigned-notdialed-calls").removeClass("disabled")
                    $("#dispo-collaps button").removeClass("active")
                    $("#submit_customer_info").attr("disabled", true)
                    $("#feedback_tab_link, #relation_tag_link").addClass("disabled")
                    $("#contact-info").click()
...
$('#skip_btn').click(function(){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            sessionStorage.removeItem("previous_number")
            $('#profile-tab, #show-callbacks-active, #show-callbacks-campaign, #show-abandonedcalls-campaign, #show-campaign-lead-bucket, #show-campaign-assigned-calls, #show-camp-requeue-lead-bucket, #show-camp-assigned-dialed-calls, #show-camp-assigned-notdialed-calls').removeClass('disabled')
...
function hangup_activity() {
    ...
    $("#btnLogMeOut, #btnNextCall").prop("disabled", true)
    $("#profile-tab, #btnNextCall,#show-callbacks-active, #show-callbacks-campaign, #show-abandonedcalls-campaign, #show-campaign-lead-bucket, #show-campaign-assigned-calls, #show-camp-requeue-lead-bucket, #show-camp-assigned-dialed-calls, #show-camp-assigned-notdialed-calls").addClass("disabled")
...
$('#btnResumePause').click(function() {
    ...
    if ((extension in session_details && Object.keys(session_details[extension]).length > 0) || btn_title !="Start Predictive Dialing"){
        if (btn_title == "Start Predictive Dialing") {
            ...
            // agent_info_vue.state = 'Idle'
            set_agentstate('Idle')
            $("#agent-callbacks-div button, #profile-tab, #show-callbacks-active, #show-callbacks-campaign, #show-abandonedcalls-campaign, #show-campaign-lead-bucket, #show-campaign-assigned-calls, #show-camp-requeue-lead-bucket, #show-camp-assigned-dialed-calls, #show-camp-assigned-notdialed-calls").removeClass("disabled")
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if (blndd_status == true && sessionStorage.getItem("ibc_popup") == "false") {
                        ...
                        $("#profile-tab, #btnNextCall, #agent-callbacks-div button, #show-callbacks-active, #show-callbacks-campaign, #show-abandonedcalls-campaign, #show-campaign-lead-bucket, #show-campaign-assigned-calls, #show-camp-requeue-lead-bucket, #show-camp-assigned-dialed-calls, #show-camp-assigned-notdialed-calls").removeClass("disabled")
                    }
                    $("#feedback_tab_link, #relation_tag_link").addClass("disabled")
                    $("#dispo-collaps button").removeClass("active")
                    $("#submit_customer_info").attr("disabled", true)
...
function getActiveCallback(campaign) {
    $.ajax({
        type: 'post',
        headers: { "X-CSRFToken": csrf_token },
...
function getCampaignTotalCallback(campaign) {
    $.ajax({
        type: 'post',
        headers: { "X-CSRFToken": csrf_token },
...
$('#show-callbacks-active, #mb-show-callbacks-active').click(function() {
    ...
    $('.breadcrumb').append('<li class="breadcrumb-item" aria-current="page"><a href="#" id="crm-home">'
        +'<i class="fas fa-home"></i></a></li>')
    getActiveCallback(campaign_name);
    ac_interval = setInterval("getActiveCallback(campaign_name)", 10000);
})
...
$('#show-callbacks-campaign').click(function() {
    ...
    $('.page-title').text(campaign_name + " Callbacks");
    $('.breadcrumb').append('<li class="breadcrumb-item" aria-current="page"><a href="#" id="crm-home">'
        +'<i class="fas fa-home"></i></a></li>')
    getCampaignTotalCallback(campaign_name)
    campcallback_interval = setInterval("getCampaignTotalCallback(campaign_name)", 10000);
})
    ...
    methods:{
        changePage(value){
            ...
            if ($('#cpm_search_by').val().trim()){
                filter_dict['column_name'] = $('input[name="cpm_column_name"]:checked').val()
...
function set_inbound_customer_detail(ic_number,contact_id){
    var dialed_uuid = session_details[extension]['dialed_uuid']
    var switch_ip = session_details[extension]["variable_sip_from_host"]
    sessionStorage.setItem('contact_id',contact_id)
    data={"ic_number":ic_number,"contact_id":contact_id,"dialed_uuid":dialed_uuid,"switch_ip":switch_ip, "callmode":callmode, "campaign_id":campaign_id}
