...
class PageNumberPagination(BasePagination):
    ...
    def paginate_queryset(self, queryset, request, view=None):
        ...
        page_number = request.POST.get(self.page_query_param, 1)
        ...
        if self.page_size_query_param:
            try:
                return _positive_int(
                    request.POST[self.page_size_query_param],
                    strict=True,
...
class LimitOffsetPagination(BasePagination):
    ...
    def get_limit(self, request):
        if self.limit_query_param:
            try:
                return _positive_int(
                    request.POST[self.limit_query_param],
                    strict=True,
    ...
    def get_offset(self, request):
        try:
            return _positive_int(
                request.POST[self.offset_query_param],
            )
        except (KeyError, ValueError):
            return 0
...
class CursorPagination(BasePagination):
    ...
    def decode_cursor(self, request):
        ...
        encoded = request.POST.get(self.cursor_query_param)
        if encoded is None:
            return None
