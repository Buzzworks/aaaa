...
from .models import (
		...
		InGroupCampaign,CampaignPriority, EmailTemplate,Daemons,PasswordChangeLogs,
)
...
def curl_addsip(request,domain):
	...
	if domain:
		...
		agent_profile={}
		ip_address = ""
	context = {'profile' : agent_profile,'ip_address':ip_address}
...
def curl_loadcc(request,domain):
	...
	if domain:
		...
		cc_user = User.objects.filter(is_active=True)
		...
		if user_file.size <= 1:
			...
			if valid:
				...
				if 	'incorrect_file' in data_dict:
					...
					data_improper.to_csv(path_or_buf=response,sep=',',float_format='%.2f',index=False,decimal=".")
					return response
				if proper_file:
...
class PendingCallbackCallView(LoginRequiredMixin, generics.ListAPIView):
	...
	def get(self, request, *args, **kwargs):
		...
		# context["user_list"] = user_hierarchy_func(request.user.id)
...
class AgentPerformanceReportView(LoginRequiredMixin,APIView):
	...
	def post(self, request, *args, **kwargs):
		...
		if selected_user:
			...
			queryset = User.objects.filter(id__in=users_agentactivity).filter(id__in=user_hierarchy_func(request.user.id,selected_user))
		else:
		...
		for user in users:
			...
			if selected_campaign:
				calldetail_cal['campaign'] = selected_campaign
			else:
...
class DNCListAPIView(LoginRequiredMixin, APIView):
	...
	def post(self, request):
		try:
			...
			dnc_columns = ['numeric', 'global_dnc']
			if dnc_file:
				if dnc_file.size <= 1:
					response_data = {}
					...
					if dnc_file.name.endswith('.csv'):
						data = pd.read_csv(dnc_file, na_filter=False,  dtype = {"numeric" : "str","global_dnc" : "str"})
					else:
...
class DNCUploadApiView(APIView):
	...
	def post(self, request):
		...
		if request.POST.get("perform_upload", ""):
			if proper_file.endswith('.csv'):
				data = pd.read_csv(cwd+proper_file, na_filter=False, dtype={"numeric" : "str"})
			else:
				data = pd.read_excel(cwd+proper_file, converters={'numeric': str})
...
class AgentLiveDataAPIView(APIView):
	...
	def get(self,request,**kwargs):
		...
		total_callbacks = CallBackContact.objects.filter(Q(user=user.extension, campaign__in=campaigns)
		...
		notifications = Notification.objects.filter(Q(user=request.user.extension,campaign__in=campaigns),
				viewed=False).count()
		total_abandonedcalls = Abandonedcall.objects.filter(Q(user=user.extension)|Q(campaign__in=campaigns,user=None)).count()
		...
		lead_count = {}
		if campaign != "":
			total_callbacks_cc = CallBackContact.objects.filter(Q(user=user.extension, campaign=campaign)
											|Q(callback_type='queue', campaign = campaign, user=None)).exclude(callmode="inbound", user=None).count()
			total_abandonedcalls_cc = Abandonedcall.objects.filter(campaign=campaign).filter(Q(user=user.extension)|Q(user=None)).count()
			active_callbacks_cc = CurrentCallBack.objects.filter(Q(user=user.extension, campaign = campaign)
...
class GetCallHistoryApiView(LoginRequiredMixin, APIView):
	...
	def post(self, request, format=None):
		...
		if unique_id:
			if contact_id:
				call_history = CallDetail.objects.filter(Q(contact_id=contact_id)|Q(uniqueid=unique_id))[:10]
			else:
				call_history = CallDetail.objects.filter(Q(customer_cid=dialed_no, campaign_name=campaign)|Q(uniqueid=unique_id))[:10]
		else:
			if contact_id:
				call_history = CallDetail.objects.filter(contact_id=contact_id)[:10]
			else:
				call_history = CallDetail.objects.filter(customer_cid=dialed_no, campaign_name=campaign)[:10]
...
class ImportDispositionData(APIView):
	...
	def post(self,request):
		...
		try:
			df = pd.read_json(file)
			model_name = [f.name for f in Disposition._meta.get_fields() if f.name not in ['id','campaign','smsgateway','emailgateway','show_dispo']]
			json_file_fields = list(df['fields'][0].keys())
			if model_name == json_file_fields:
...
class SmsGatewayCreateEditApiView(LoginRequiredMixin, APIView):
	...
	def get(self, request, pk="", **kwargs):
		...
		if non_admin_user:
			...
			campaigns = user_campaigns.values('id','name')
		else:
			campaigns = Campaign.objects.values('id','name')
		...
		if pk:
			...
			elif sms_gateway.gateway_mode == '1':
				selected_campaigns = Campaign.objects.filter(whatsapp_gateway__id=pk).values_list('id', flat=True)
			# selected_campaigns = Campaign.objects.filter(sms_gateway__id=pk).values_list('id', flat=True)
	...
	def post(self, request,pk=None,**kwargs):
		...
		if sms_gateway_serializer.is_valid():
			...
			if request.POST.get('gateway_mode') == "0":
			...
			elif request.POST.get('gateway_mode') == "1":
				Campaign.objects.filter(whatsapp_gateway__id=pk).update(whatsapp_gateway=None)
				Campaign.objects.filter(pk__in=request.POST.getlist("campaign")).update(whatsapp_gateway = sms_gateway.id)
			### Admin Log ####
			return Response()
...
class GetAgentBreakwisetotal(LoginRequiredMixin,APIView):
	...
	def post(self,request):
		if request.user:
			...
			user_activity = AgentActivity.objects.filter(user=request.user,created__date=date.today())
